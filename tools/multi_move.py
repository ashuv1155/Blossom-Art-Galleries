...
moves = {
    "BAG 10": "Traditional Garlands & Haars",
    "BAG 11": "Traditional Garlands & Haars",
    "BAG 12": "Traditional Garlands & Haars",
    "BAG 294": "Artificial Flora",
    "BAG 296": "Artificial Flora",
    "BAG 297": "Artificial Flora",
    "BAG 298": "Artificial Flora"
}
...
for csv_file in [csv_path1, csv_path2]:
    ...
    for row in rows:
        if row['Product ID'] in moves:
            row['Category'] = moves[row['Product ID']]
    ...
    if csv_file == csv_path1:
        # Calculate new counts from the updated rows
        for r in rows:
            if r['Category'] in new_counts:
                new_counts[r['Category']] += 1

print("Updated CSVs!")
print(f"Traditional Garlands & Haars now has {new_counts['Traditional Garlands & Haars']} items.")
print(f"Artificial Flora now has {new_counts['Artificial Flora']} items.")
...
def update_card(match):
    ...
    if bag_match:
        ...
        if bag_id in moves:
            new_cat = moves[bag_id]
            new_slug = cat_name_to_slug[new_cat]
            # Update data-category
            card = re.sub(r'data-category="[^"]*"', f'data-category="{new_slug}"', card)
            # Update hidden category label
            card = re.sub(r'<span class="uppercase tracking-widest text-primary font-bold">.*?</span>', f'<span class="uppercase tracking-widest text-primary font-bold">{new_cat}</span>', card)
    return card
...
print("SUCCESS! Completed multiple category moves.")
